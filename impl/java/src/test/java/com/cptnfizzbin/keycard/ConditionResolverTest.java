...
public class ConditionResolverTest {
    ...
    public void testNestedFieldCondition() {
        ...
        assertTrue(resolver.evaluate(article, Map.of("owner", Map.of("name", Conditions.op("$ne", "frank")))));
        assertFalse(resolver.evaluate(article, Map.of("owner", Map.of("name", Conditions.op("$ne", "james")))));
    }
