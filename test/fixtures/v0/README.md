...
```yaml
version: "0.1"          # required — a v0 PolicyDefinition, per SPEC_V0.md §4
name: name of the test suite
description: string       # optional
meta:                      # optional — see SPEC_V0.md §4.2
  anyAction: ...
  anySubject: ...
  actions: [...]
  subjects: [...]
...
instance's value (a `SubjectRef`, per §6.2.2) and makes the check
...
`version` exceeds it is skipped, not failed, mirroring the compatibility rule in
SPEC_V0.md §4.1 (same `MAJOR`, `MINOR` no higher than what's supported; `PATCH`
never matters). This is automatic: once fixtures for a newer `MINOR` version are
...
rule-evaluation algorithm (§6.2.2), and the condition/operator language (§5). It
does **not** cover the purely construction-time validation requirements —
malformed rule tuples (§4.3), `version` incompatibility (§4.1), a
both-sides-wildcarded rule carrying a `Conditions` element (§4.3, §6.2.1),
`meta.actions`/`meta.subjects`/`meta.operators`
catalog-coverage enforcement (§4.2.2, §4.2.3), a `meta.operators` entry with
nothing registered for it (§4.2.3), or a duplicate operator name across the
built-ins and whatever custom operators were supplied (§4.2.3)
...
`tests` field (SPEC_V0.md §4.4, added in `1.1.0`): that field is data a policy
document carries about *itself*, for an implementation's own test-runner to
execute (§6.2.3); this directory's `tests:` sequences are the conformance
...
- `03-wildcards.yaml` — `_ANY_` default, a custom wildcard token, and disabling
  the wildcard mechanism via `null` (§4.2.1, §6.2.2 property 4).
...
- `09-custom-operators.yaml` — unregistered, uncataloged custom operators always
  evaluate to `false` (§5.5). The cataloged-but-never-registered case is no
...
- `11-worked-example.yaml` — an end-to-end mirror of the spec's own Appendix
  policy.
- `12-tests-field.yaml` — a `1.1.0` policy (SPEC_V0.md §4.4) whose own embedded
